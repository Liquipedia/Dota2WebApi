$(document).ready(function() {
	window.usecodemirror = false;
	if((typeof window.codemirror == 'object')) {
		window.usecodemirror = true;
	}
	...
	var insertFullMatchDetails = function(context) {
		var selection,
			matchIDs,
			vars;
		if(window.usecodemirror) {
			selection = window.codemirror.getSelection().replace(/\s+$/, '').replace(/^\s+/, '');
		} else {
		...
		addInsertDialog(matchIDs, {
			...
			insertCallback: function(event, ui) {
				...
				if(window.usecodemirror) {
					window.codemirror.setValue(window.codemirror.getValue().replace(replaceText, s));
				} else {
